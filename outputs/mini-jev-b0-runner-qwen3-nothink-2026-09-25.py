#!/usr/bin/env python3
"""W5 B0: Banking77 high-cardinality probe. 30 queries x 1-of-77 labels x 3 runs.
Same frozen judge (qwen2.5:3b, temp 0). Scoring case-INSENSITIVE (B0-30 gold
lowercase vs Capitalized label - exact-match would false-negative a correct answer).
Returns per-item + accuracy + confusion clusters. No interpretation."""
...
def api(host, payload, timeout=300):
...
host = pick_host()
print(f"host={host} model={MODEL} runs={RUNS} cases={len(CASES)} labels={len(LABELS)}")
rows = []
t0 = time.time()
for cid, text, gold in CASES:
    for run in range(1, RUNS + 1):
        t1 = time.time()
        try:
            r = api(host, {"model": MODEL, "prompt": PROMPT.format(labels=", ".join(LABELS), text=text),
                           "stream": False, "think": False, "options": {"temperature": 0, "num_predict": 40}})
            lat = time.time() - t1
            out = r.get("response", "").strip().splitlines()
            out = [l.strip().strip(" \"'") for l in out if l.strip()]
            got = out[-1] if out else ""
            hit = LOWER.get(got.lower())
            verdict = ("HIT" if hit and hit.lower() == gold.lower()
                       else ("MISS->" + hit if hit else "UNPARSEABLE"))
        except Exception as e:
            lat = time.time() - t1
            got, verdict = "", "ERROR:" + str(e)[:50]
...
print(f"\nBATCH: {len(rows)} calls in {round(total,1)}s | accuracy {hits}/{len(rows)} = {round(hits/len(rows)*100,1)}%")
from collections import Counter
print("confusion:", dict(Counter((r["gold"], r["verdict"]) for r in rows if r["verdict"] != "HIT")))
out = {"host": host, "model": MODEL, "method": "generative 1-of-77 judge, temp 0, case-insensitive scoring",
       "note": "B0-30 gold lowercase in handover vs Capitalized label; scored case-insensitively",
       "batch_s": round(total, 1), "accuracy": f"{hits}/{len(rows)}", "rows": rows}
open("/var/folders/kl/2pdh9p0j585dv40l78p7wkch0000gn/T/opencode/b0_raw.json", "w").write(json.dumps(out, indent=1))
print("saved b0_raw.json")
