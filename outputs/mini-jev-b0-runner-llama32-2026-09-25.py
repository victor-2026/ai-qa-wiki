...
MODEL = "llama3.2:3b"
RUNS = 3
OUTP = "/Users/victor/Projects/ai-qa-wiki/outputs/mini-jev-b0-llama32-3b-raw-2026-09-25.json"
...
def main():
    ...
    rows = []
    try:
        prev = json.load(open(OUTP)).get("rows", [])
        done = {(r["id"], r["run"]) for r in prev}
        rows = [r for r in prev if r.get("model", MODEL) == MODEL]
        print(f"resumed: {len(rows)} rows kept", flush=True)
    except Exception:
        done = set()
    t0 = time.time()
