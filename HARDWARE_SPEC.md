...
## Model Recommendations (2026-06-19)

Анализ железа и оптимальные модели для текущей конфигурации.

### PC-224 (64GB RAM, GPU 6GB VRAM)

Ключевой bottleneck: GPU 6GB VRAM — 70b модели только CPU.

| Модель | Размер | Зачем |
|--------|--------|-------|
| **qwen2.5:14b** (уже есть) | ~9GB | Русский #1 — лучший quality/speed |
| **deepseek-coder-v2:lite** | ~9GB | Кодинг — сильнее чем r1:14b |
| **qwen2.5-coder:14b** | ~9GB | Альтернатива для генерации тестов |
| **llama3.3:70b-q4_K_M** | ~25GB | CPU-only (~3-5 tok/s), сложные задачи на английском |
| **minicpm-v** | ~3GB | Vision — быстрее llama3.2-vision, влезает в VRAM 6GB |

### MacBook (16GB Intel)

Только малые модели ≤8B Q4.

| Модель | Размер | Зачем |
|--------|--------|-------|
| **qwen2.5:7b** (Q4) | ~4GB | Русский (быстрее чем 3b, влезает в 16GB) |
| **llama3.2:3b** (уже есть) | ~2GB | Быстрые тесты |

### Что НЕ стоит

- ❌ **llama3.3:70b full** (~40GB) — займёт почти всю RAM, Docker/Qdrant умрут
- ❌ **deepseek-r1:14b** — уступил qwen2.5:14b по русскому, дублирование
- ❌ **kimi-k2.6:cloud** — облачная, не Ollama

### Итог

`qwen2.5:14b` — король для русского. Добавить `deepseek-coder-v2:lite` для кода. `70b-q4_K_M` — только если нужно deep reasoning и готов ждать 3-5 tok/s.
